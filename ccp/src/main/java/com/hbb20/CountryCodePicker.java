...
public class CountryCodePicker extends RelativeLayout {
    ...
    //this will be "AU,IN,US"
    String customMasterCountriesParam,customMasterCountriesNameParam, excludedCountriesParam;
...
//        Log.d(TAG, "getEditText_registeredCarrierNumber");
    ...
    void refreshCustomMasterList() {
        ...
        if (customMasterCountriesParam == null || customMasterCountriesParam.length() == 0) {
            ...
            List<CountryData> localCCPCountryList = new ArrayList<>();

            for (String nameCode : customMasterCountriesParam.split("#")) {
                String[] nameCodeLan = nameCode.split("/");
                CCPCountry ccpCountry = CCPCountry.getCountryForNameCodeFromLibraryMasterList(getContext(), getLanguageToApply(), nameCodeLan[0]);
                if (ccpCountry != null) {
                    ccpCountry.setName(nameCodeLan[1]);
                    ccpCountry.setLanguageCode(nameCodeLan[2]);
                    CountryData data = new CountryData(ccpCountry.nameCode,  ccpCountry.phoneCode,  ccpCountry.name,  ccpCountry.englishName,  ccpCountry.flagResID,ccpCountry.languageCode);
                    localCCPCountryList.add(data);
                }
            }
            ...
            if (localCCPCountryList.size() == 0) {
                ...
                List<CCPCountry> localCCPCountryLista = new ArrayList<>();
                for (CountryData dd : localCCPCountryList) {
                    CCPCountry ccpCountry = new CCPCountry(dd.nameCode, dd.phoneCode, dd.name, dd.englishName, dd.flagResID,dd.languageCode);
                    localCCPCountryLista.add(ccpCountry);
                }
                customMasterCountriesList = localCCPCountryLista;
            }
        }
    ...
    public void setLoadedLibraryMasterListLanguage(CountryCodePicker.Language loadedLibraryMasterListLanguage) {
        CCPCountry.loadedLibraryMasterListLanguage = loadedLibraryMasterListLanguage;
    }


    public void setLoadedLibraryMaterList(List<CCPCountry> loadedLibraryMaterList) {
    ...
    public String getSelectedLanguageCode() {
        return getSelectedCountry().languageCode;
    }
