...
import com.android.vadify.ui.chat.call.CallActivity.Companion.memberIds
...
abstract class AudioBaseClass<ActivityBinding : ViewDataBinding> :
...
//    }
    ...
    fun createLocalTracks() {
        localAudioTrack = LocalAudioTrack.create(this, true)
        getFrontCameraId()?.let { cameraCapturer = CameraCapturer(this, it) }
        cameraCapturer?.let { localVideoTrack = LocalVideoTrack.create(this, false, it) }
        (applicationContext as VadifyApplication).localVideoTrack = localVideoTrack
    }

    fun updataLocalTrack(it: LocalVideoTrack) {
        localVideoTrack = it
        thumbnail_video_view1.mirror = true
        localVideoTrack?.addSink(thumbnail_video_view1)
        localVideoView = thumbnail_video_view1
    }
        ...
        if (checkSelfPermissionCompat(Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED
            ...
            if ((applicationContext as VadifyApplication).videoCallType is VadifyApplication.VideoCallType.GroupCall) {
                ...
                intent.extras?.getString(ROOM_ID)?.let {
                    bindNetworkState(
                        viewModel.groupCallRequestApi(memberid, CallActivity.AUDIO, it)
                    )
                }
            }
            else
                intent.extras?.getString(CallActivity.ANOTHER_USERID)?.let {
                    bindNetworkState(
                        viewModel.callSingleRequestApi(CallActivity.AUDIO, it)
                    )
                }
        }
    }
